...
def read_grid(file_path):
    ...
    content = []
    f = open(file_path, 'r')
    # content = [[int(num) for num in line.split(',')]for line in f]
    content = [csv_line_to_list(line) for line in f]
    # for line in f:
    #     content.append(csv_line_to_list(line))
    f.close()
    return content
...
def get_pixel_at(pixel_grid, i, j):
    ...
    if i >= 0 and i < len(pixel_grid) and j >= 0 and j < len(pixel_grid[0]):
...
def average_of_surrounding(pixel_grid, i, j):
    ...
    for n in range(i-1, i+2, 1):
        for m in range(j-1, j+2, 1):
            pixel_sum = pixel_sum + get_pixel_at(pixel_grid, n, m)
...
def blur(pixel_grid):
    ...
    """
    blurred_pixel_grid = [
        [0 for x in range(len(pixel_grid[0]))] for y in range(len(pixel_grid))]
    for i in range(0, len(blurred_pixel_grid)):
        for j in range(0, len(blurred_pixel_grid[0])):
            blurred_pixel_grid[i][j] = average_of_surrounding(pixel_grid, i, j)

    return blurred_pixel_grid
...
pixel_grid = blur(input_grid)
# print(pixel_grid)

# Step E: Write the result to both output files
if ext == '.txt':
    write_grid(out_grid_filename, pixel_grid)
else:
    write_image(out_image_filename, pixel_grid)
print("Program done")
...
# I wrote everything by myself
# # with some help from course materials and stackoverflow.
